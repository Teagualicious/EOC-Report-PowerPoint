...
- Phase 4 (2026-07-13) — AI-native interfaces (Claude Business readiness):
  ...
  - Round 2 (search pivot contract): the Excel search now behaves as a strict typed pivot table — columns render in the exact typed order (dims and metrics interleaved; implied dims first, default metrics appended last), and the auto KPI summary boxes were removed (they added unrequested aggregate "boxes"/groupings above the results). modSearch.bas WriteResults/ParseTerms rewritten around a unified column sequence (mColSeq); grammar docs and Search-sheet hint updated; VBA/dashboard contract locked by static tests. **Needs the Windows search drill re-run** (typed-order searches, e.g. "Impressions, Campaign, Client" and "Client, Campaign, Zip Code").
